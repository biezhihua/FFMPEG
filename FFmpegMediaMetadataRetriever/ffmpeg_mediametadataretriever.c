...
void convert_image(State *state, AVCodecContext *pCodecCtx, AVFrame *pFrame, AVPacket *avpkt, int *got_packet_ptr);
            ...
            if (pkt->stream_index == state->video_stream) {
                ...
                if (!is_supported_format(codec_id, pix_fmt)) {
                    ...
                    if (got_frame) {
                        ...
                        convert_image(state, state->video_st->codec, frame, &convertedPkt, &got_packet);
                ...
                *got_frame = 0;
                ...
                if (*got_frame) {
                    if (desired_frame_number == -1 ||
                        ...
                        av_init_packet(pkt);
                        convert_image(state, state->video_st->codec, frame, pkt, got_frame);
                        break;
                    }
                }
            } else {
...
int get_frame_at_time(State **ps, int64_t timeUs, int option, AVPacket *pkt) {
    ...
    if (got_packet) {
        const char *filename = "/Users/biezhihua/Desktop/one.png";
